    ...
    _event_handler(new EventHandler())
{
    ...
    _time_last_lolclick = _time_last_update;
}
...
void MainWindow::loadSettings(){
    ...
    if(settings.contains("lol-analog")) {
        ui->checkBoxLolAnalog->setChecked(settings.value("lol-analog").toBool());
    }
    on_checkBoxRightClick_toggled(ui->checkBoxRightClick->isChecked());
...
void MainWindow::saveSettings()
{
    ...
    settings.setValue("lol-analog", ui->checkBoxLolAnalog->isChecked());
...
bool MainWindow::handleButtons(QList<int> buttons)
{
    qint64 now = QDateTime::currentMSecsSinceEpoch();
 ...
 * \return button pressed, held, released
 */
